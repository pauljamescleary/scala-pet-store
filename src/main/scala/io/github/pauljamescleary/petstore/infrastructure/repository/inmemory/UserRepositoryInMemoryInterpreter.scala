...
class UserRepositoryInMemoryInterpreter[F[_]: Applicative] extends UserRepositoryAlgebra[F] {
  ...
  def update(user: User)(implicit b: B): F[Option[User]] = user.id.traverse{ id =>
    cache.update(id, user)
    user.pure[F]
  }

  def get(id: Long)(implicit b: B): F[Option[User]] = cache.get(id).pure[F]

  def delete(id: Long)(implicit b: B): F[Option[User]] = cache.remove(id).pure[F]
