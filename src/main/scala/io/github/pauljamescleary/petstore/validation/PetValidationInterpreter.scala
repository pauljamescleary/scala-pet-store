...
class PetValidationInterpreter(repository: PetRepositoryAlgebra[IO])
    extends PetValidationAlgebra[IO] {
  ...
  def exists(petId: Option[Long]): EitherT[IO, ValidationError, Unit] =
    EitherT {
      petId match {
        case Some(id) =>
